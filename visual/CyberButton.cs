...
{
    // Кнопка Modern 2026: премиальный дизайн с эффектом стекла, объёмом,
    // плавными hover-анимациями, ripple-эффектом и светящимся акцентом.
    public class CyberButton : Control, IButtonControl
    {
        ...
        public CyberButton()
        {
            ...
            _anim.Tick += (s, e) =>
            {
                if (_isHovered && _hover < 1f) _hover = Math.Min(1f, _hover + 0.18f);
                else if (!_isHovered && _hover > 0f) _hover = Math.Max(0f, _hover - 0.15f);
        ...
        {
            base.OnMouseDown(e);
            if (e.Button == MouseButtons.Left) { _rippleC = e.Location; _rippleR = 2f; _rippleA = 180f; _ripple.Start(); }
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            // 1) Наружное цветное свечение ПОД кнопкой
            if (eff > 0.01f)
            {
                int[] al = { 36, 22, 12 };
            ...
            // 2) Плотный собственный градиент: верх осветлён, низ = база
            var topCol = CyberPalette.Mix(CustomBaseColor, CyberPalette.CardBgTop, 0.70f);
            var botCol = CyberPalette.Mix(CustomBaseColor, Color.Black, 0.14f);
            using (var bg = new LinearGradientBrush(rect, topCol, botCol, 90f))
                g.FillPath(bg, path);

            g.SetClip(path);

            // 3) Мягкий внутренний свет сверху-центр для объёма
            ...
            {
                hl.CenterColor = CyberPalette.Alpha(Color.White, 32);
            ...
            if (eff > 0.01f)
                using (var warm = new LinearGradientBrush(rect,
                    CyberPalette.Alpha(AccentColor, (int)(eff * 52)),
                    CyberPalette.Alpha(AccentColor, (int)(eff * 18)), 90f))
            ...
            if (_active)
            {
                var bar = new Rectangle(rect.X + 2, rect.Y + 6, 4, rect.Height - 12);
                using (var glow = new LinearGradientBrush(new Rectangle(bar.X, bar.Y, 16, bar.Height),
                    CyberPalette.Alpha(AccentColor, 140), Color.Transparent, 0f))
                    g.FillRectangle(glow, bar.X, bar.Y, 16, bar.Height);
                using (var bb = new LinearGradientBrush(bar, CyberPalette.AccentGlow, AccentColor, 90f))
                    g.FillRectangle(bb, bar);
            }
            g.ResetClip();

            // 7) Рамка-фаска + стеклянный верхний край 1px
            using (var border = new Pen(CyberPalette.Mix(CyberPalette.BorderColor, AccentColor, eff), 1.2f + 0.5f * eff))
                g.DrawPath(border, path);
            using (var topHi = new Pen(CyberPalette.Alpha(Color.White, 52 + (int)(eff * 28)), 1f))
            ...
            Color txt = eff > 0.5f ? Color.White
                : CyberPalette.Mix(CyberPalette.TextPrimary, Color.White, _hover * 0.35f);
