...
namespace UselessChecker
{
    public partial class Form1 : Form
    {
        ...
        private async Task RunDnsCacheScanAsync(Label statusLabel, CyberButton scanBtn)
        {
            ...
            scanBtn.Text = "Анализ...";
            statusLabel.Text = "Запрос системного DNS-кэша...";
            ...
            try
            {
                var foundTraces = await Task.Run(() =>
                {
                    ...
                    var allDnsEntries = new List<string>();
                    ...
                    if (process != null)
                    {
                        ...
                        // Улучшенный парсинг: ищем все DNS записи по шаблону
                        var lines = output.Split(new[] { "\r\n", "\n" }, StringSplitOptions.RemoveEmptyEntries);
                        string currentRecordName = "";
                        bool inRecordBlock = false;
                        
                        foreach (var line in lines)
                        {
                            string trimmedLine = line.Trim();
                            
                            // Определяем начало записи DNS
                            if (trimmedLine.StartsWith("Record Name", StringComparison.OrdinalIgnoreCase))
                            {
                                var parts = trimmedLine.Split(new[] { ':' }, 2);
                                if (parts.Length > 1)
                                    currentRecordName = parts[1].Trim().ToLowerInvariant().TrimEnd('.');
                                inRecordBlock = true;
                            }
                            else if (inRecordBlock && trimmedLine.Length > 0 && !trimmedLine.Contains(":") && trimmedLine.Contains("."))
                            {
                                currentRecordName = trimmedLine.ToLowerInvariant().TrimEnd('.');
                            }
                            
                            if (inRecordBlock && !string.IsNullOrEmpty(currentRecordName))
                            {
                                // Проверяем имя записи
                                foreach (var keyword in CheatSignatures.DnsCheatKeywords)
                                {
                                    if (currentRecordName.Contains(keyword))
                                    {
                                        if (!matches.Contains(currentRecordName))
                                            matches.Add(currentRecordName);
                                        break;
                                    }
                                }
                                
                                // Проверяем значения в строках с ":"
                                            ...
                                            foreach (var keyword in CheatSignatures.DnsCheatKeywords)
                                            {
                                                if (value.Contains(keyword))
                                                {
                                                    if (!matches.Contains(value))
                                                        matches.Add(value);
                                                    if (!matches.Contains(currentRecordName) && !string.IsNullOrEmpty(currentRecordName))
                                                        matches.Add(currentRecordName);
                                                    break;
                                                }
                                            }
                    ...
                    return matches.Distinct().OrderBy(x => x).ToList();
                });

                if (foundTraces.Count > 0)
                {
                    statusLabel.Text = $"Найдено совпадений: {foundTraces.Count}";
                    statusLabel.ForeColor = CyberPalette.AccentNeon;
                    
                    string report = string.Join("\n", foundTraces.Select((t, i) => $"{i + 1}. {t}"));
                    await ShowCustomMessageBoxAsync(
                        $"В системном DNS-кэше обнаружены следы обращения к серверам читов/софта:\n\n{report}\n\n" +
                        $"Это подтверждает факт запуска соответствующего ПО или посещения связанных ресурсов.\n" +
                        $"Для очистки кэша выполните в командной строке: ipconfig /flushdns",
                        "Результат анализа DNS", "Warning");
                }
                else
                {
                    statusLabel.Text = "Угроз не обнаружено";
                    statusLabel.ForeColor = CyberPalette.AccentEmerald;
                    await ShowCustomMessageBoxAsync(
                        "Системный DNS-кэш чист.\n\nСледов обращения к известным серверам читов и софта не найдено.\n\n" +
                        "Рекомендация: для профилактики выполните очистку кэша командой:\n" +
                        "ipconfig /flushdns",
            ...
            catch (Exception ex)
            {
                ...
                statusLabel.ForeColor = CyberPalette.AccentRedMuted;
                await ShowCustomMessageBoxAsync(
                    $"Не удалось проанализировать DNS-кэш:\n{ex.Message}\n\n" +
